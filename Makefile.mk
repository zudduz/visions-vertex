...
deploy:
	...
	PATH=$HOME/.local/bin:$$PATH (uv export --no-hashes --no-header --no-dev --no-emit-project --no-annotate > .requirements.txt 2>/dev/null || \
	PATH=$HOME/.local/bin:$$PATH uv export --no-hashes --no-header --no-dev --no-emit-project > .requirements.txt) && \
